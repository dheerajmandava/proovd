...
export async function createEvent(eventData) {
    try {
        ...
        // Sanitize inputs
        const sanitizedData = Object.assign(Object.assign({}, eventData), { siteId, name: eventData.name ? sanitizeInput(eventData.name) : undefined, data: eventData.data ? {
 ...
 */
export async function getEventsBySiteId(siteId, limit = 20, skip = 0, type) {
    try {
        // Convert string ID to ObjectId
        const objectId = new mongoose.Types.ObjectId(siteId);
        // Create query
        const query = { siteId: objectId };
        if (type) {
            query.type = type;
        }
        // Fetch events
 ...
 */
export async function getEventCountBySiteId(siteId, type) {
    try {
        // Convert string ID to ObjectId
        const objectId = new mongoose.Types.ObjectId(siteId);
        // Create query
 ...
 */
export async function getRecentEventsForNotifications(siteId, limit = 5) {
    try {
        // Convert string ID to ObjectId
        const objectId = new mongoose.Types.ObjectId(siteId);
        // Fetch recent events
 ...
 */
export async function deleteEventsBySiteId(siteId) {
    try {
        // Convert string ID to ObjectId
        const objectId = new mongoose.Types.ObjectId(siteId);
        // Delete events
