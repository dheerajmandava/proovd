...
function convertToWebsiteWithAnalytics(website) {
    return {
 ...
 */
export const getWebsiteById = cache(async (id) => {
 ...
 */
export const getWebsitesByUserId = cache(async (userId) => {
...
export async function updateWebsiteSettings(id, settings) {
    ...
    if (settings.position)
        website.settings.position = settings.position;
    if (settings.delay !== undefined)
        website.settings.delay = parseInt(settings.delay.toString(), 10);
    if (settings.displayDuration !== undefined)
        website.settings.displayDuration = parseInt(settings.displayDuration.toString(), 10);
    if (settings.maxNotifications !== undefined)
        website.settings.maxNotifications = parseInt(settings.maxNotifications.toString(), 10);
    if (settings.theme)
        website.settings.theme = settings.theme;
    if (settings.displayOrder)
        website.settings.displayOrder = settings.displayOrder;
    if (settings.randomize !== undefined)
        website.settings.randomize = Boolean(settings.randomize);
    if (settings.initialDelay !== undefined)
        website.settings.initialDelay = parseInt(settings.initialDelay.toString(), 10);
    if (settings.loop !== undefined)
        website.settings.loop = Boolean(settings.loop);
    if (settings.customStyles !== undefined)
        website.settings.customStyles = settings.customStyles;
    // Update other fields
    if (settings.name)
        website.name = settings.name;
    if (settings.allowedDomains)
        website.allowedDomains = settings.allowedDomains;
    if (settings.status)
        website.status = settings.status;
    // Save changes
    await website.save();
 ...
 * @returns Updated website or null if not found
 */
export async function incrementWebsiteImpressions(id, count = 1) {
    if (!id || !mongoose.Types.ObjectId.isValid(id) || count <= 0)
        return null;
    await connectToDatabase();
    const website = await Website.findById(id);
    if (!website)
 ...
 * @returns Updated website or null if not found
 */
export async function incrementWebsiteClicks(id, count = 1) {
    if (!id || !mongoose.Types.ObjectId.isValid(id) || count <= 0)
        return null;
    await connectToDatabase();
    const website = await Website.findById(id);
 ...
 */
export async function getWebsiteByApiKey(apiKey) {
    if (!apiKey)
        return null;
    try {
        await connectToDatabase();
 ...
 */
export async function getWebsiteVerification(id, userId) {
    if (!id || !mongoose.Types.ObjectId.isValid(id))
        return null;
    await connectToDatabase();
 ...
 * @returns Updated website or null if not found
 */
export async function updateWebsiteVerificationMethod(id, method, token) {
    if (!id || !mongoose.Types.ObjectId.isValid(id))
        return null;
    await connectToDatabase();
    ...
    const website = await Website.findByIdAndUpdate(id, { $set: { verification } }, { new: true }).lean();
    return website;
}
/**
 ...
 * @returns Updated website or null if not found
 */
export async function updateWebsiteVerificationStatus(id, isVerified, reason) {
    if (!id || !mongoose.Types.ObjectId.isValid(id))
        return null;
    return withDatabaseConnection(async () => {
        const website = await Website.findById(id);
 ...
 */
export async function getWebsitesRaw(userId) {
    if (!userId || !mongoose.Types.ObjectId.isValid(userId))
        return [];
    await connectToDatabase();
