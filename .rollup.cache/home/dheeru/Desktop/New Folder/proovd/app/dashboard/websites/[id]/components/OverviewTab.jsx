...
export default function OverviewTab({ websiteId }) {
        ...
        async function fetchData() {
            try {
                ...
                if (notificationsData && notificationsData.notifications && Array.isArray(notificationsData.notifications)) {
                    const formatted = notificationsData.notifications.map((notification) => ({
                        id: notification.id,
    ...
    const renderConversionRate = () => {
        ...
        if ((_a = websiteData.cachedStats) === null || _a === void 0 ? void 0 : _a.conversionRate) {
      ...
      <div className="card bg-base-100 shadow-xl mb-8">
        <div className="card-body">
          ...
          <div className="form-control w-full">
            ...
            <div className="mt-2">
              <CopyButton textToCopy={installationCode} className="btn btn-sm btn-ghost gap-2" iconClassName="h-4 w-4" label="Copy to clipboard"/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body">
          <div className="flex items-center justify-between">
            ...
            <Link href={`/dashboard/websites/${websiteData.id}/notifications`} className="btn btn-sm btn-ghost gap-2">
              View All 
              <ArrowTopRightOnSquareIcon className="h-4 w-4"/>
            </Link>
          </div>

          {formattedNotifications.length > 0 ? (<div className="overflow-x-auto">
              <table className="table table-zebra">
                ...
                <tbody>
                  {formattedNotifications.map((notification) => (<tr key={notification.id}>
