...
export default function WebsiteTabs({ websiteId, activeTab }) {
    const tabs = [
    ...
    return (<div className="tabs tabs-bordered mb-6">
      {tabs.map((tab) => (<Link key={tab.id} href={tab.href} className={`tab ${activeTab === tab.id ? 'tab-active' : ''}`}>
          {tab.label}
        </Link>))}
    </div>);
}
