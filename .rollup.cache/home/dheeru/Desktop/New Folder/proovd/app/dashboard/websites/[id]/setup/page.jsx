...
export default function WebsiteSetupPage() {
    ...
    const [verificationTab, setVerificationTab] = useState(VerificationMethod.DNS);
    ...
    useEffect(() => {
        async function fetchWebsite() {
            ...
            try {
                ...
                if ((_a = data.verification) === null || _a === void 0 ? void 0 : _a.method) {
    ...
    useEffect(() => {
        ...
        const fetchVerificationInfo = async () => {
            try {
                ...
                if (!response.ok)
                    throw new Error('Failed to fetch verification info');
                const data = await response.json();
                // Update website with the latest verification data
                setWebsite((prev) => (Object.assign(Object.assign({}, prev), { verification: data.verification, verificationInstructions: data.instructions })));
            }
            catch (err) {
    ...
    }, [params.id, (_a = website === null || website === void 0 ? void 0 : website.verification) === null || _a === void 0 ? void 0 : _a.status]);
    const handleCopy = (text, field) => {
        navigator.clipboard.writeText(text)
            .then(() => {
            setCopySuccess(Object.assign(Object.assign({}, copySuccess), { [field]: true }));
            setTimeout(() => {
                setCopySuccess(Object.assign(Object.assign({}, copySuccess), { [field]: false }));
            }, 2000);
        })
            .catch(err => {
    ...
    const changeVerificationMethod = async (method) => {
        try {
            ...
            if (!response.ok)
                throw new Error('Failed to update verification method');
            const data = await response.json();
            // Update website with the new verification method
            setWebsite((prev) => (Object.assign(Object.assign({}, prev), { verification: Object.assign(Object.assign({}, prev.verification), { method: data.method, status: 'pending' }) })));
    ...
    const verifyDomain = async () => {
        try {
            ...
            if (data.success) {
                setWebsite((prev) => (Object.assign(Object.assign({}, prev), { verification: Object.assign(Object.assign({}, prev.verification), { status: 'verified' }), status: 'verified' })));
            }
        }
        catch (err) {
    ...
    const isVerified = ((_b = website.verification) === null || _b === void 0 ? void 0 : _b.status) === 'verified';
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {isVerified ? (<Check className="h-5 w-5 text-green-500"/>) : (<span className="flex h-5 w-5 items-center justify-center rounded-full bg-amber-500 text-white text-xs">1</span>)}
          ...
          <CardContent>
            {isVerified ? (<Alert className="bg-green-50 border-green-500">
                ...
                <Tabs value={verificationTab} onValueChange={changeVerificationMethod}>
                      ...
                      <div className="rounded-md bg-muted p-4 space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <div className="col-span-2 font-mono bg-background p-1 rounded overflow-x-auto">
                            {(_c = website.verification) === null || _c === void 0 ? void 0 : _c.token}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="rounded-md bg-muted p-4 space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <div className="col-span-2 font-mono bg-background p-1 rounded">
                            {(_d = website.verification) === null || _d === void 0 ? void 0 : _d.token}
                          </div>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                  ...
                  <TabsContent value={VerificationMethod.META}>
                    <div className="space-y-4">
                      ...
                      <CodeSection code={`<meta name="proovd-verification" content="${(_e = website.verification) === null || _e === void 0 ? void 0 : _e.token}" />`} language="html"/>
                    </div>
                  </TabsContent>
                </Tabs>
                
                {verificationResult && (<Alert className={`mt-4 ${verificationResult.success ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'}`}>
                    {verificationResult.success ? (<Check className="h-4 w-4 text-green-500"/>) : (<AlertTriangle className="h-4 w-4 text-red-500"/>)}
          ...
          <CardFooter>
            {!isVerified && (<Button onClick={verifyDomain} disabled={verifying} className="w-full">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <CodeSection code={scriptTag} language="html"/>
              
              {!isVerified && (<Alert variant="destructive">
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button disabled={!isVerified} onClick={() => window.location.href = `/dashboard/websites/${params.id}/notifications`}>
