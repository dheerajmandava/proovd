...
import { HomeIcon, ChartBarIcon, CreditCardIcon, QuestionMarkCircleIcon, GlobeAltIcon, ClockIcon } from '@heroicons/react/24/outline';
    ...
    return (<div className="drawer-side z-20">
      ...
      <aside className="w-72 h-screen flex flex-col bg-base-100 border-r border-base-200">
        ...
        <div className="px-4 py-4 border-b border-base-200">
          ...
          <WebsiteSelector activeWebsiteId={activeWebsiteId} onSelect={handleWebsiteSelect}/>
        </div>
        ...
        <div className="px-3 py-4 flex-grow overflow-y-auto">
          <ul className="menu bg-base-100 rounded-box gap-1">
            ...
            return (<li key={item.name}>
                  <Link href={item.href} className={`${isActive ? 'active font-medium' : ''} hover:bg-base-200 transition-colors duration-150`}>
                    <item.icon className="h-5 w-5" aria-hidden="true"/>
                    {item.name}
                    {item.badge && (<span className="badge badge-sm badge-primary">{item.badge}</span>)}
                  </Link>
                </li>);
        })}
          </ul>
