...
export default function WebsiteSelector({ activeWebsiteId, onSelect }) {
    ...
    if (isLoading) {
        return (<div className="select select-bordered w-full h-12 flex items-center justify-between opacity-70">
        <span className="loading loading-spinner loading-sm"></span>
        <span>Loading websites...</span>
      </div>);
    }
    if (error) {
        return (<div className="alert alert-error p-2 text-xs">
        <span>{error}</span>
        <button onClick={() => window.location.reload()} className="btn btn-xs btn-ghost">Retry</button>
      </div>);
    }
    if (websites.length === 0) {
        return (<Link href="/dashboard/websites/new" className="btn btn-primary btn-outline w-full gap-2">
        <PlusIcon className="h-5 w-5"/>
        Add Your First Website
      </Link>);
    }
    return (<div className="dropdown w-full">
      <div tabIndex={0} role="button" onClick={() => setIsOpen(!isOpen)} className="btn btn-outline w-full justify-between font-normal border-base-300 bg-base-100 text-base-content shadow-sm">
        {activeWebsite ? (<div className="flex items-center gap-2 truncate">
          ...
          </div>) : (<span>Select a website</span>)}
      ...
      <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow-lg bg-base-100 rounded-box w-full mt-1 border border-base-200">
        {websites.map((website) => (<li key={website._id}>
            <button onClick={() => handleSelect(website._id)} className={`flex items-center ${website._id === activeWebsiteId ? 'active font-medium' : ''}`}>
