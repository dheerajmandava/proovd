...
export async function GET(request) {
    ...
    try {
        ...
        return new NextResponse(JSON.stringify({ success: true, message: 'Analytics processing completed successfully' }), { status: 200 });
    }
    catch (error) {
        console.error('Error in analytics job:', error);
        // Return error response
        return new NextResponse(JSON.stringify({ error: 'Failed to run analytics job' }), { status: 500 });
    }
}
/**
