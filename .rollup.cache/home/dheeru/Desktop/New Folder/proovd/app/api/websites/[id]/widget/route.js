...
export async function GET(request, props) {
    ...
    try {
        ...
        if (website.status !== 'active' && website.status !== 'verified') {
            return NextResponse.json({ error: 'Website is not active' }, { status: 403 });
        }
        ...
        if (refererDomain) {
            ...
            const isDomainAllowed = baseDomain === websiteBaseDomain ||
        ...
        return NextResponse.json({ error: 'Failed to serve widget' }, { status: 500 });
    }
}
/**
