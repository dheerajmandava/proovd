...
function StatsCard({ title, value, icon, description, isLoading = false }: StatsCardProps) {
  return (
    <div className="rounded-xl border bg-card p-6 shadow-sm hover:shadow transition-shadow duration-200">
...
export default function RealTimeStats({ websiteId, initialStats, onLoadingChange }: RealTimeStatsProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <StatsCard
          ...
          description={`Your notifications have been seen ${displayStats.totalImpressions} times`}
          isLoading={isLoading}
        />
        <StatsCard
          title="Total Clicks"
          ...
          description={`Your notifications have been clicked ${displayStats.totalClicks} times`}
          isLoading={isLoading}
        />
        <StatsCard
          ...
          icon={<ChartBarIcon className="h-4 w-4" />}
          description={`${displayStats.conversionRate.toFixed(2)}% of impressions resulted in clicks`}
          isLoading={isLoading}
        />
      </div>
    </div>
