...
function DomainVerificationForm({
  verificationData,
  websiteId,
  ...
  const [selectedMethod, setSelectedMethod] = useState<VerificationMethod>(
    verificationData.verification.method
  );
  const [isVerifying, setIsVerifying] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Handle method change
  async function handleMethodChange(method: VerificationMethod) {
    try {
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      setError((err as Error).message);
      onVerificationComplete(false, (err as Error).message);
    } finally {
      setIsVerifying(false);
    }
  }
        ...
        <form onSubmit={handleVerify}>
          <div className="form-control mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => handleMethodChange(VerificationMethod.DNS)}
                className="btn btn-sm btn-primary"
                disabled={selectedMethod === VerificationMethod.DNS}
              >
...
export default function VerifyWebsitePage() {
  ...
  async function handleVerify(success: boolean, message: string) {
    try {
      setIsVerifying(true);
      setError(null);
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {isLoading ? (
        ...
        <>
          {verificationData.verification.status === VerificationStatus.VERIFIED ? (
            <div className="card bg-base-100 shadow-lg">
              <div className="card-body">
