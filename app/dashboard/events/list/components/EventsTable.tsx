...
export default function EventsTable({ websiteId }: EventsTableProps) {
      ...
      case 'view':
        return (
          <div>
            ...
            {data.pageUrl && (
              <span className="block text-xs opacity-70 truncate max-w-xs">{data.pageUrl}</span>
            )}
          </div>
        );
      default:
  ...
  const getEventTypeLabel = (type: string) => {
    switch (type) {
      case 'signup':
        return (
          <span className="badge badge-info">Signup</span>
        );
      case 'purchase':
        return (
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        <button 
          className={`btn btn-sm ${eventType === null ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => handleTypeFilter(null)}
        >
          All
        </button>
        <button 
          className={`btn btn-sm ${eventType === 'signup' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => handleTypeFilter('signup')}
        >
          Signups
        </button>
        <button 
          className={`btn btn-sm ${eventType === 'purchase' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => handleTypeFilter('purchase')}
        >
          Purchases
        </button>
        <button 
          className={`btn btn-sm ${eventType === 'view' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => handleTypeFilter('view')}
        >
          Views
        </button>
      </div>
      ...
      {loading && events.length === 0 ? (
        ...
        <div className="bg-base-100 p-8 text-center rounded-lg">
