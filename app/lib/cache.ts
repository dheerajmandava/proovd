 ...
 */
export function createCache<T>(ttl: number): Map<string, T> {
  ...
  cache.set = function(key: string, value: T): Map<string, T> {
    expiryTimes.set(key, Date.now() + ttl);
    return originalSet.call(this, key, value);
  };
  
  const originalGet = cache.get;
  cache.get = function(key: string): T | undefined {
  ...
  cache.delete = function(key: string): boolean {
    expiryTimes.delete(key);
    return originalDelete.call(this, key);
  };
  
  const originalClear = cache.clear;
  cache.clear = function(): void {
    expiryTimes.clear();
