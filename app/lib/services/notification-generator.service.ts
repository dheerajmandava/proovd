 ...
 */
export async function generateSignupNotification(event: any) {
  try {
 ...
 */
export async function generatePurchaseNotification(event: any) {
  try {
    const { siteId, data } = event;
 ...
 */
export async function generateViewersNotification(siteId: string, pageUrl: string, count: number) {
  if (count < 2) return null; // Don't show for less than 2 viewers
 ...
 */
export async function processEventsForNotifications(siteId: string) {
  try {
    // Convert string ID to ObjectId
    const objectId = new mongoose.Types.ObjectId(siteId);
...
function createSignupMessage(data: any) {
  const name = data?.userName || 'Someone';
  return `${name} just signed up`;
}

function createPurchaseMessage(data: any) {
  const name = data?.userName || 'Someone';
  const product = data?.productName || 'an item';
  
  return `${name} just purchased ${product}`;
}

function createViewersMessage(count: number) {
