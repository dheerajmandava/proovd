 ...
 * @returns Promise resolving to verification success status
 */
async function verifyByMeta(domain: string, token: string): Promise<boolean> {
    ...
    https.get(url, (res) => {
      ...
      res.on('end', () => {
        ...
        const metaTagRegex = new RegExp(
          ...
          `<meta[^>]*content=["']${token}["'][^>]*name=["']proovd-verification["'][^>]*>`, 
          'i'
        );
 ...
 */
export async function verifyDomainWithDetails(
  domain: string,
  verification: VerificationDetails
  ...
  try {
    ...
    return {
      isVerified: false,
      method: VerificationMethod.DNS,
      reason: `Verification error: ${(error as Error).message}. Please try again in a few minutes.`
    };
  }
} 