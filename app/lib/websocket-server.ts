...
export class PulseWebSocketServer {
  ...
  private websiteStats: Map<string, WebsiteStats> = new Map();
  private ownerConnections: Map<string, Set<any>> = new Map();
  ...
  private handleConnection(ws: any, request: any): void {
    try {
      ...
      if (isOwner) {
        ...
        this.ownerConnections.get(websiteId)?.add(ws);
        
        ws.on('close', () => {
          this.ownerConnections.get(websiteId)?.delete(ws);
          if (this.ownerConnections.get(websiteId)?.size === 0) {
   ...
   */
  private async enrichClientWithLocation(client: ClientConnection): Promise<void> {
    try {
   ...
   * @param client Client connection
   */
  private sendStatsToClient(client: ClientConnection): void {
    try {
   ...
   */
  private sendStatsToDashboard(websiteId: string, stats: WebsiteStats): void {
    try {
   ...
   * @param websiteId Website ID
   */
  private updateWebsiteStats(websiteId: string): void {
    try {
      ...
      if (activeClients.length > 0) {
        const totalTimeOnPage = activeClients.reduce((sum, client) => 
          sum + (client.metrics?.timeOnPage || 0), 0);
      ...
      const totalScrollPercentage = activeClients.reduce((sum, client) => 
        sum + (client.metrics?.scrollPercentage || 0), 0);
      ...
      stats.totalClicks = activeClients.reduce((sum, client) => 
        sum + (client.metrics?.clickCount || 0), 0);
  ...
  private broadcastStats(): void {
    try {
      ...
      const websiteClients: Record<string, ClientConnection[]> = {};
   ...
   */
  public getActiveUsersCount(websiteId: string): number {
    const stats = this.websiteStats.get(websiteId);
   ...
   */
  public getStats(websiteId: string): WebsiteStats | null {
