 ...
 */
function getVerificationInstructionsServer(
  domain: string,
...
export async function GET(
  ...
  try {
    ...
    if (!website.verification) {
      ...
      await updateWebsiteVerificationMethod(
        id, 
    ...
    const instructions = getVerificationInstructionsServer(
      website.domain,
...
export async function POST(
  ...
  try {
    ...
    if (body.method) {
      ...
      let methodEnum: VerificationMethod;
      switch (methodLower) {
        case 'dns':
      ...
      const updatedWebsite = await updateWebsiteVerificationMethod(
        id, 
        methodEnum,
        website.verification.token // Keep the existing token
      );
    ...
    const verificationResult = await verifyDomainWithDetails(
      website.domain,
      website.verification
    );
    
    // Update verification status
    const updatedWebsite = await updateWebsiteVerificationStatus(
      id,
