...
export class StockStatus implements WidgetInterface {
  private container: HTMLElement | null = null;
  private element: HTMLElement | null = null;
  private currentStock: number = 0;
  private config: WidgetConfig;
  private options: StockStatusOptions;

  constructor(config: WidgetConfig) {
    this.config = config;
    
    // Default options
    const defaultOptions: StockStatusOptions = {
  ...
  update(data: StockData): void {
    if (!this.element || typeof data.currentStock !== 'number') return;
   ...
   */
  private showWidget(text: string, color: string): void {
   ...
   */
  private animateStockChange(): void {
    if (!this.element) return;
   ...
   */
  private applyPulseAnimation(): void {
    if (!this.element) return;
