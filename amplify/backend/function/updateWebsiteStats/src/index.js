...
exports.handler = async (event) => {
  try {
    // Extract website ID from event
    const { websiteId } = event.arguments;
    ...
    const sessionsResponse = await docClient.query({
    ...
    // Calculate averages
    const avgTimeOnPage = activeUsers > 0 ? totalTimeOnPage / activeUsers : 0;
    const avgScrollPercentage = activeUsers > 0 ? totalScrollPercentage / activeUsers : 0;
